<?php
  ...
  if(isset($_POST["Search"])){
    ...
    if((strcmp($_SESSION['uType'], 'advisor') == 0)){
      ...
      $query = "select * from student, personalinfo WHERE unid = '$input' and universid = advisorid and advisorid = '$_SESSION[uID]' and (NOT applied_to_grad = 3)";
      $result = mysqli_query($dbc, $query);
      if(mysqli_num_rows($result) > 0){
        echo '<center><h4>Student Found</h4></center><div class="advbasicdata">';  
        
        echo '<table style="width:100%">';
        echo '<tr><th>University ID</th><th>Advisor</th><th>GPA</th><th>Program</th><th>Major</th><th>Applied to Grad?</th></tr>';
        while($row = $result->fetch_assoc()){ 
          echo "<tr><td><center>" . $row["unid"]. "</center></td><td><center>" . $row["ftname"] . " " . $row["ltname"] ."</center></td><td><center>" . $row["gpa"]. "</center></td><td><center>" . $row["program"]. "</center></td><td><center>" . $row["major"] . "</center></td>";
          if($row["applied_to_grad"] == 0){
            echo "<td><center>No</center></td></tr>";
          }
          else{
            echo "<td><center>Yes</center></td></tr>";
          }
        }
        echo '</table></div>';
        echo '<hr />';
        ...
        if ($result->num_rows > 0){
          ...
          while($row = $result->fetch_assoc()){
              echo "<tr><td><center>" . $row["yeartaken"]. "</center></td><td><center>" . $row["semester"]. "</center></td><td><center>" . $row["crseid"]. "</center></td><td><center>" . $row["title"]. "</center></td><td><center>" . $row["grade"]. "</center></td><td><center>" . $row["chours"]. "</center></td></tr>";
          }
          echo '</table></div>';
          echo '<hr />';
          ...
          if ($result->num_rows > 0){
            ...
            while($row = $result->fetch_assoc()){
                echo "<tr><td><center>" . $row["courseid"]. "</center></td><td><center>" . $row["title"]. "</center></td><td><center>" . $row["credits"]. "</center></td></tr>";
            }
            echo '</table></div>';
    ...
    // THIS IS FOR GS AND ADMIN : ALLOWS THEM TO VIEW EVERY USER AND THEIR ADVISOR ID'S. CANNOT CLEAR A USER'S THESIS.        
      ...
      $query = "select * from student, personalinfo WHERE unid = '$input' and (NOT applied_to_grad = 3) and universid = advisorid";
      $result = mysqli_query($dbc, $query);
      if(mysqli_num_rows($result) > 0){
        echo '<center><h4>Student Found</h4></center><div class="basicdata">';  
        echo '<br><form action="" method="post">';
        echo  '<label for="assignAdvi">Assign Student an Advisor by ID</label><br>';
        ...
        echo	'</form><br>';
        echo '<table style="width:100%">';
        echo '<tr><th>University ID</th><th>Advisor ID</th><th>GPA</th><th>Program</th><th>Major</th><th>Applied to Grad?</th></tr>';
        while($row = $result->fetch_assoc()){
          echo "<tr><td><center>" . $row["unid"]. "</center></td><td><center>" . $row["ftname"] . " " . $row["ltname"] . "</center></td><td><center>" . $row["gpa"]. "</center></td><td><center>" . $row["program"]. "<center></td><td><center>" . $row["major"] . "</center></td>";
          $stuID = $row["unid"];
          if($row["applied_to_grad"] == 0){ 
  ...
  //SHOW ALL STUDENTS : THIS IS ONLY SHOWN IF A STUDENT HAS NOT YET BEEN SEARCHED
    ...
    if((strcmp($_SESSION['uType'], 'gs') == 0 || strcmp($_SESSION['uType'], 'admin') == 0)){
      ...
      if ($result->num_rows > 0){
        ...
        while($row = $result->fetch_assoc()){
          echo "<tr><td><center>" . $row["ftname"]. "</center></td><td><center>" . $row["ltname"]. "</center></td><td><center>" . $row["unid"]. "</center></td><td><center>" . $row["advisorid"]. "</center></td><td><center>" . $row["program"]. "</center></td></tr>";
        }
        echo '</table></div><br>';
      }
      else{
    ...
    //DISPLAY ONLY USERS FOR THE SELECTED ADVISOR
      ...
      if ($result->num_rows > 0){
        ...
        while($row = $result->fetch_assoc()){
          echo "<tr><td><center>" . $row["ftname"]. "</center></td><td><center>" . $row["ltname"]. "</center></td><td><center>" . $row["unid"]. "</center></td><td><center>" . $row["program"]. "</center></td></tr>";
        }
        echo '</table></div><br>';
      }
      else{
